...
def run_prediction_test(bout_df, pagerank_df, fie_df, weapon, gender, category,
    ...
    for bout in bouts_x1.itertuples():
        ...
        if (f_fie is not None and o_fie is not None and
            f_pr  is not None and o_pr  is not None):
            total += 1
    ...
    pr_acc = pr_correct / total if total > 0 else None
    fie_de_acc = fie_de / de_total if de_total > 0 else None
    pr_de_acc = pr_de / de_total if de_total > 0 else None
...
def run_all_prediction_tests(pagerank_df, fie_df, weapon, gender, category, date, div, log_lines):
    ...
    log_lines.append(f"  {'Season Pair':<13} {'FIE':<6} {'PR':6} {'Diff':<6} {'DE FIE':<8} {'PR FIE':<8} {'DE Diff':<8}")
    log_lines.append(f"  {'-'*65}")
    
    results = []
    for sx, sx1 in season_pairs:    # for all seasons we want to test
        r = run_prediction_test(
            bout_df, pagerank_df, fie_df, weapon, gender, category, sx, sx1, log_lines
        )
        results.append(r)
    
    # summary
    results_df = pd.DataFrame(results).dropna(subset=['fie_acc', 'pr_acc'])
...
with open('data_analysis/prediction_test.txt', 'w', encoding='utf-8') as f:
    f.write('\n'.join(pred_log))
