...
comparison_df = pd.read_csv('data_analysis/merged_rankings.csv')

# Create data_analysis/merged_spearman_comparisions.csv -----------------------------------------------
spearman_records = []
for (weapon, gender, category, season), group in comparison_df.groupby(
    ['weapon', 'gender', 'category', 'fie_season']):
    
    if len(group) < 10:     # for early seasons with very few fencers
        continue
    
    rho, pvalue = spearmanr(group['pagerank_rank_common'], group['fie_rank_common'])
    rho_ts, pvalue_ts = spearmanr(group['ts_rank_3sigma_common'], group['fie_rank_common'])
    rho2, pvalue2 = spearmanr(group['pagerank_rank_common'], group['ts_rank_3sigma_common'])
...
spearman_df.to_csv('data_analysis/merged_spearman_comparisions.csv', index=False)
print(f"Saved {len(spearman_df)} merged Spearman results")
...
no_covid = spearman_df[spearman_df['fie_season'] != '2020/2021']
